...
Moving onto TF-IDF. This is the term frequency multiplied by the log of the total number of documents divided by the number of documents containing the term. Its used to see how important a word is to a document in our corpus. A common word is in many documents, so DF is large, so N/DF is near 1, so the log of that is near 0, meaning near zero weight. A rare word is in fewer documents, so N/DF is big, which means a big IDF. So IDF down-weights common words and boosts rare ones.
...
This is the phrase queries stage where I extended each posting in our inverted index to store the positions of the term within the document. The whole point is to use phrases over single word queries. So we had word then operator then word, but that just means if we were to pass in computer systems it wouldn't necessarily check for where computer systems together are shown, but more so every document where computer and systems is shown. So for a phrase "computer systems vector" a match at pos means computer at pos, system at pos+1 and vector at pos+2, with enumerate giving us the offset/index value.

To get position I just enumerated tokens to get index and value and appended it to our inverted index. So now the result of our json would be the term, which documents it belongs to and its positions, and term frequency is now derived from the length of that rather than being directly given. That is the tradeoff, rather than storing just one number per posting, we now store a whole list of positions, so it's a storage cost that grows with how often the term occurs (O(n) space where n is the occurrences, rather than one posted number). Getting the tf back out is still cheap since len is O(1).


One issue with these phrase queries however, is that stop word removal drops words like the and of, which causes some breakage in this type of retrieval. Dropping those words shifts the positions that the phrase check relies on, so a phrase that had stop words in it can't match properly, and that matters because with a phrase we care about the exact wording.

## Stage 5

This is the BM25 stage!!! stands for best matching 25, which is a search algorithm that uses key words to rank document relevance. It is generally superior to TF IDF because it adds two things. First is term frequency saturation, which just means the repetition of keyword terms has diminishing returns. Second is document length normalization, so long documents don't just outrank short ones.

This is done through the constants k1 and b. k1 controls the saturation. If it's high the score scales up each time the word is repeated, but if it's low the score hits a limit, or like the name says, it saturates quicker. If it's 0 then tf is basically ignored and the score just defaults to base IDF. b on the other hand handles the document length normalization. The closer it is to 1 the stronger the length penalty, so if a document has the same term frequency but is a shorter document it ranks higher, and the closer to 0 the more length is ignored.

To actually see BM25 do this, I compared it against TF IDF on the query "ai computer". TF IDF ranked "Algorithmic bias" (6720 tokens) first, but BM25 pushed "AI data center" (1665 tokens) to first instead, so the shorter denser document rose above the long one. The 5th result also changed, from "Outline of machine learning" (3041 tokens) to "Algorithm selection" (847 tokens, tiny). That is the length normalization and saturation removing the long document bias that TF IDF had. I kept both scorers pluggable so I could run the same query through each and compare.

As for why I picked the values I did, b = 0.75 is a default value and k1 of 1.5 is also practically a default. Changing these values did change the output ordering. BM25 is used a lot in RAG retrieval too for AI agents and LLMs. I used it in lorechain with the langchain framework to get the semantic RAG, even if it was overkill for that corpus. I think mainly I just wanted to understand all this information retrieval that is now pipelined in agent frameworks, and that was the purpose.

## Stage 6

Last stage. This one was packaging it up so the repo reads as a finished thing rather than something I abandoned halfway.

First the CLI. My entry point was a bare input("search: ") prompt followed by a second prompt asking bm25 or tfidf. That works but it isn't really a tool, it's a debug hook. So I wrote cli.py using argparse with three subcommands, index, search and eval. Search takes --method, -k and --phrase as flags, so everything is one line now instead of answering prompts one at a time.

One thing I hit here that is worth writing down. query.py opens index.json at module level, so it reads the file the moment it gets imported. That means if cli.py imported query at the top, then running cli.py index on a fresh clone would crash before it ever got to build the index, which is exactly the one situation where the index is supposed to be missing. So I import query inside the search function instead of at the top, and left a comment saying why, because otherwise that looks like a mistake to anyone reading it later, me included.

I also changed ranking and bm25 to return the scores and not just the document names. They were running heapq.nlargest over the dict keys, so I changed it to nlargest over result.items() which hands back (doc, score) pairs. Printing the score matters more than I expected. Without it you can't see the ranking doing anything, you just get a list of names in some order and you have to trust it. With the scores you can watch the gaps, and in the tf-idf output the gap between first and second is huge while in BM25 everything is bunched up, which is saturation showing up visually.

Then the evaluation. I picked 5 queries and judged relevance by hand by actually opening the documents rather than guessing from the titles, then measured precision at 5, so out of the top 5 results how many were actually relevant. My rule was that a document counts if it is about the topic or discusses it properly, and a passing mention does not count. That rule ends up doing real work, because Outline of machine learning is a page of links that names nearly every term in the corpus, so it matches almost everything and helps nobody, and under that rule it is never relevant.

BM25 came out at 0.56 mean and tf-idf at 0.48, and BM25 won 2 queries and tied the other 3, so it never actually lost to tf-idf. But the number I nearly reported wrong is the ceiling. There are only 2 documents in this entire corpus about anomaly detection, so precision at 5 on that query cannot go above 0.40 no matter how perfect the ranking is. BM25 scored exactly 0.40 there, which is a perfect result that looks like a failure if you just read the number on its own. So I added a best possible column to the eval output, and the honest headline is 0.56 against a ceiling of 0.80, not 0.56 out of 1.00. Worth remembering that a metric can have a ceiling below 1 and you will fool yourself if you don't check it.

The failure cases were more interesting than the score. The main one is polysemy. On the query neural network, the anomaly detection article scores well because it is stuffed with the word network, except it means computer networks and sensor networks and network intrusion, not neural ones. Lexical search matches characters, it has no concept of one string meaning two different things, and that is exactly the gap dense embeddings fill. Which is the whole reason hybrid retrieval exists, so the project ended up demonstrating its own motivation, which was satisfying.

Last was the README, since it is the first thing anyone sees and mine was two sentences. What the project is, the architecture as a pipeline diagram, setup and usage, the tf-idf against BM25 comparison with real numbers and document lengths side by side, the evaluation table with the ceiling column, the known failure cases, and a list of what I would add next.

And that is where I'm stopping. I set out to understand the retrieval layer sitting underneath RAG instead of importing it and trusting it, and I have that now. I can read a BM25 call in a framework and know what k1 and b are doing, why the posting lists have to be sorted, and what the positional index is costing me. Next thing is the dense half.
