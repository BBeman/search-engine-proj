...
This is the BM25 stage!!! This stands for best matching 25, which is a search algorithm that uses key words to rank document relevance. It is generally superior to TF-IDF because it adds two things. First is term frequency saturation, which just means the repetition of keyword terms has diminishing returns. Second is document length normalization, so long documents don't just outrank short ones.
